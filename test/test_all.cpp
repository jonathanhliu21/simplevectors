...
TEST(XYMagnitudeAngleMatchTest2D, TestMagnitudeGivenXY) {
  ...
  double magn_r = std::ceil(magn * 1000.0) / 1000.0;
...
TEST(XYMagnitudeAngleMatchTest2D, TestAngleGivenXY) {
  ...
  for (auto testcase : tests) {
    ...
    double ang_r = std::ceil(ang * 1000.0) / 1000.0;

    EXPECT_EQ(ang_r, testcase[2]);
  }
}

TEST(ComponentTest2D, TestConvertToPair) {
...
TEST(ComponentTest2D, TestConvertToStruct) {
  Vector2D vector(3.5, -6.2);
...
TEST(ComponentTest2D, TestNormalize) {
  Vector2D vector(3, 4);
